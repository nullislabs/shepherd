//! Chassis-store acceptance tests against the composed
//! `nexum_sdk_test::MockHost` - the chassis touches only the local
...
use nexum_sdk::chassis::{
    ConditionalSource, Gates, Journal, NEXT_BLOCK_PREFIX, NEXT_EPOCH_PREFIX, RetryAction,
    RetryLedger, Tick, WATCH_PREFIX, WatchRef, WatchSet, watch_key,
};
...
fn ledger_backoff_gates_the_watch_on_the_epoch_clock() {
    ...
    let key = seeded_watch(&host);
    let watch = WatchRef::parse(&key).unwrap();
    let ledger = RetryLedger::new(&host);
...
/// chassis passes the stored params verbatim and the tick it judged
/// the gates by.
#[test]
