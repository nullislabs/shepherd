//! MockVenue acceptance tests: the scripted venue driving the chassis
//! materialiser (multi-tick retry, backoff, and outage scenarios the
...
use nexum_sdk::chassis::{ConditionalSource, Journal, Tick, WatchRef, WatchSet, watch_key};
use nexum_sdk::host::{Fault, LocalStoreHost as _, RateLimit};
use shepherd_sdk::cow::{
    CowApiError, CowHost, OrderRejection, PollOutcome, materialise, order_uid_hex,
};
...
#[test]
fn chassis_retries_a_transient_rejection_then_submits() {
...
#[test]
fn chassis_backs_off_on_rate_limit_and_submits_after_the_gate() {
    let host = MockHost::with_venue();
    seed_watch(&host);
    let order = submittable_order();
    host.cow_api
    ...
    let t0 = sample_tick();
    let source = ready_source(&order);
    materialise(&host, &source, &t0).unwrap();
    assert_eq!(host.cow_api.call_count(), 1);
    ...
    materialise(&host, &source, &gated).unwrap();
    ...
    materialise(&host, &source, &clear).unwrap();
    assert_eq!(host.cow_api.call_count(), 2);
    assert!(
        Journal::submitted(&host)
            .contains(&client_uid(&order))
...
#[test]
fn chassis_survives_a_venue_outage_and_submits_on_recovery() {
    let host = MockHost::with_venue();
    let key = seed_watch(&host);
...
#[test]
fn chassis_drops_the_watch_on_a_scripted_permanent_rejection() {
    let host = MockHost::with_venue();
    seed_watch(&host);
    let order = submittable_order();
    host.cow_api
    ...
    materialise(&host, &ready_source(&order), &sample_tick()).unwrap();

    assert!(host.store.is_empty(), "watch and gates must go");
    assert_eq!(host.cow_api.call_count(), 1);
}

/// Chassis rows written through the composed host stay invisible to a
...
#[test]
fn chassis_sweep_ignores_sibling_namespace_watches() {
    let host = MockHost::with_venue();
    seed_watch(&host);
    let sibling = host.store.namespaced("other-module");
    assert!(sibling.is_empty(), "chassis rows must not leak across");
    sibling
        .set(
