//! Watch materialisation: the poll-loop composition conditional-
//! commitment modules share.
//!
//! [`materialise`] walks the chassis watch set, polls each gate-ready
...
//! journal as the idempotency guard and the chassis [`RetryLedger`]
...
use nexum_sdk::chassis::{
    ConditionalSource, Gates, Journal, RetryLedger, Tick, WatchRef, WatchSet,
};
...
pub fn materialise<H, S>(host: &H, source: &S, tick: &Tick) -> Result<(), Fault>
where
    H: CowHost,
...
fn submit_ready<H: CowHost>(
    ...
    match host.submit_order(tick.chain_id, &body) {
        ...
        Err(err) => {
            ...
            RetryLedger::new(host).apply(watch, action, tick.epoch_s)?;
