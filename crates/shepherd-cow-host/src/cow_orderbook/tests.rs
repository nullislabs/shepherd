...
#[test]
fn from_config_applies_extension_override() {
...
"#,
    ...
    assert!(
        ...
        "extension override applied, got {}",
        api.base_url(),
    );
}
...
async fn submit_order_propagates_orderbook_envelope() {
    ...
    // so the WIT adapter can forward `api` to the cow-api-error
    // rejected case. The string
