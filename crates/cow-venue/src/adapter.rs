...
//! ([`TimedFetch`](videre_sdk::transport::TimedFetch)). Orderbook
...
//! the pre-sign path), `http-timeout-ms` (per-request bound, default
//! 30000).
...
/// Default per-request timeout bound in milliseconds.
...
/// [`TimedFetch`](videre_sdk::transport::TimedFetch) bound.
mod export {
    use videre_sdk::VenueAdapter;
    use videre_sdk::transport::TimedFetch;
    ...
    impl VenueAdapter for CowAdapter {
        ...
        fn quote(body: Vec<u8>) -> Result<Quotation, VenueError> {
            ...
            super::quote_with(&TimedFetch::new(WasiFetch, config.timeout), &config, &body)
        }

        fn submit(body: Vec<u8>) -> Result<SubmitOutcome, VenueError> {
            let config = super::config()?;
            super::submit_with(&TimedFetch::new(WasiFetch, config.timeout), &config, &body)
        }

        fn status(receipt: Vec<u8>) -> Result<IntentStatus, VenueError> {
            let config = super::config()?;
            super::status_with(
                &TimedFetch::new(WasiFetch, config.timeout),
                &config,
                &receipt,
            )
        }
...
mod tests {
    ...
    fn requests_ride_the_configured_timeout_bound() {
        ...
        let timed = TimedFetch::new(&fetch, config.timeout);
