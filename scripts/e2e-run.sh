...
# price-alert and balance-tracker live in the nexum-runtime repo since the
# carve; the e2e scenario expects their wasms prebuilt from a sibling
# checkout (../nexum-runtime) into this repo's target dir.
...
(
    cd "$REPO_ROOT"
    nohup "$REPO_ROOT/target/release/shepherd" \
