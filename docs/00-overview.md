...
| | `cow::run` - the shared poll-loop composition: run the keeper watch set, poll a `Poller`, submit `Ready` orders behind the `submitted:` journal guard and retry ledger |
