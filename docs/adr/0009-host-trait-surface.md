...
# Host trait surface: per-capability traits plus a supertrait, with a `strategy.rs` / `lib.rs` split

> **The error envelope is superseded by [ADR-0011](0011-per-interface-typed-errors.md):** the single `HostError` / `HostErrorKind` record is replaced by per-interface typed errors over a shared `fault` vocabulary. The trait-surface and `strategy.rs` / `lib.rs` decisions below still hold; read `HostError` as its successor types.
...
3. **Per-module `strategy.rs` (pure, wit-independent logic, unit tests against `MockHost`) plus `lib.rs` (per-cdylib `wit_bindgen::generate!` glue, the `WitBindgenHost` impl, and the `Guest` dispatch).** Colocating hundreds of lines of strategy with the mechanical adapter obscures both.

## Consequences

- Strategy code is testable in native Rust without `wasm32-wasip2`; every module ships a `MockHost` unit-test suite.
