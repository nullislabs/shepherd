...
load_env() {
    [[ -f "$ENV_FILE" ]] || die "shepherd/scripts/.env not found. Run: cp shepherd/scripts/env-template shepherd/scripts/.env && \$EDITOR shepherd/scripts/.env"
    set -a
    # shellcheck disable=SC1090
    source "$ENV_FILE"
    set +a
    [[ -n "${RPC_URL_SEPOLIA:-}"      ]] || die "RPC_URL_SEPOLIA unset in shepherd/scripts/.env"
    [[ -n "${RPC_URL_SEPOLIA_HTTP:-}" ]] || die "RPC_URL_SEPOLIA_HTTP unset in shepherd/scripts/.env"
...
render_engine_config() {
    local src="$REPO_ROOT/shepherd/engine.e2e.toml"
    local dst="$REPO_ROOT/shepherd/engine.e2e.local.toml"
